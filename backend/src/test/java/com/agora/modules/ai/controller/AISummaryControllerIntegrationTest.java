...
class AISummaryControllerIntegrationTest {
    ...
    @Autowired SintesisIaRepository sintesisIaRepository;
    ...
    @MockitoBean OllamaAIProvider ollamaAIProvider;
    @MockitoBean MockAIProvider mockAIProvider;
    ...
    @BeforeEach
    void setUp() {
        sintesisIaRepository.deleteAll();
    ...
    @Test
    void generatesSummaryHistoryAndFallback() throws Exception {
        String student = login("estudiante@agora.com");
        String teacher = login("docente@agora.com");
        Long attemptId = start(student);
        when(ollamaAIProvider.generateSummary(any()))
                .thenReturn(new AIProviderResult("Sintesis generada por Ollama", "llama3.1:8b"))
                .thenThrow(new IllegalStateException("Ollama no disponible"));
        when(mockAIProvider.generateSummary(any()))
                .thenReturn(new AIProviderResult("Sintesis fallback mock", MockAIProvider.MODEL));

        mockMvc.perform(post("/api/v1/attempts/" + attemptId + "/ai/summary")
                        .header("Authorization", bearer(student)).contentType(MediaType.APPLICATION_JSON)
                        .content("{\"instrucciones\":\"Sintetizar avance academico\"}"))
                .andExpect(status().isCreated())
                .andExpect(jsonPath("$.fueExitosa").value(true))
                .andExpect(jsonPath("$.modeloUtilizado").value("llama3.1:8b"));

        mockMvc.perform(post("/api/v1/attempts/" + attemptId + "/ai/summary")
                        .header("Authorization", bearer(teacher)).contentType(MediaType.APPLICATION_JSON)
                        .content("{\"instrucciones\":\"Docente no genera\"}"))
                .andExpect(status().isForbidden());

        mockMvc.perform(post("/api/v1/attempts/" + attemptId + "/ai/summary")
                        .header("Authorization", bearer(student)).contentType(MediaType.APPLICATION_JSON)
                        .content("{\"instrucciones\":\"Forzar fallback por Ollama no disponible\"}"))
                .andExpect(status().isCreated())
                .andExpect(jsonPath("$.fueExitosa").value(true))
                .andExpect(jsonPath("$.modeloUtilizado").value("mock-ai-provider-v1"));

        mockMvc.perform(get("/api/v1/attempts/" + attemptId + "/ai/summary")
