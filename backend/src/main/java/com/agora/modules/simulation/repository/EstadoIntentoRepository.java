...
public interface EstadoIntentoRepository extends JpaRepository<EstadoIntento, Long> {
    ...
    @Query("""
            SELECT ei FROM EstadoIntento ei
            JOIN FETCH ei.estadoEmocional ee
            JOIN FETCH ei.intento i
            WHERE i.id IN :attemptIds
            """)
    List<EstadoIntento> findByIntentoIdIn(@Param("attemptIds") Collection<Long> attemptIds);
}
